...
class OpponentModel:
    ...
    def __init__(self, profile):
        ...
        self._frequencyModel = DistributionBasedFrequencyOpponentModel \
            ...
            #FrequencyOpponentModel.create().With(profile.getProfile().getDomain(), newResBid=None)
    ...
    def getOpponentUtility(self, potentialBid: Bid):
        ...
        for counter,issue in enumerate(self._issues):
            #todo issue is not necessarily in bid
            sum += self._issueWeights[counter] * self._frequencyModel._get_value_val(issueValues[issue], self._frequencyModel._bidFrequencies[issue])
                   #self._frequencyModel._getFraction(issue, self._frequencyModel.val(issueValues[issue]))
        return sum
    ...
    def updateIssueWeights(self):
        ...
        for counter,issue in enumerate(self._issues):
            ratio = self._frequencyModel._get_value_val(currentBidBidValues[issue], self._frequencyModel._bidFrequencies[issue])/self._frequencyModel._get_value_val(previousBidValues[issue], self._frequencyModel._bidFrequencies[issue])
